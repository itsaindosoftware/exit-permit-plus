...
class AttendanceMatchingService
{
    ...
    public function loadRowsWithSource(?UploadedFile $uploadedFile = null): array
    {
        ...
        $networkPath = trim((string) config('attendance.source_network_path', ''));

        if ($networkPath !== '') {
            return $this->loadRowsFromNetworkDirectory($networkPath);
        }
     ...
     * @return array{rows: Collection<int, array<string, mixed>>, source: array<string, mixed>}
     */
    private function loadRowsFromNetworkDirectory(string $directory): array
    {
        if (!is_dir($directory)) {
            throw ValidationException::withMessages([
        ...
        if (@copy($latestFile, $tempPath) === false) {
            @unlink($tempPath);
