...
export default function AuthenticatedLayout({ header, children }) {
    ...
    const notifications = usePage().props.notifications ?? { unread: [], unread_count: 0, exit_permit_approval_count: 0, schedule_car_count: 0, reimbursement_approval_count: 0 };
    const [showSidebar, setShowSidebar] = useState(false);
    const [showUserMenu, setShowUserMenu] = useState(false);
    const userMenuRef = useRef(null);
    const isRatna = String(user?.email ?? '').toLowerCase() === 'hrga-01@example.com';
    const isSisca = String(user?.email ?? '').toLowerCase() === 'payroll.hr@example.com';
                        ...
                        {menuItems.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {item.badgeCount > 0 && (
                                        <span className="flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-orange-500 px-1.5 text-[10px] text-white">
